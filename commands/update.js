...
const UPDATE_IMG = 'https://i.ibb.co/7NyMLkMq/IMG-20260921-WA0562.jpg';
...
async function performUpdate(zipUrl, onProgress) {
    ...
    try {
        ...
        tmpZip = path.join(os.tmpdir(), `nezuko-update-${Date.now()}.zip`);
        fs.writeFileSync(tmpZip, Buffer.from(res.data));

        onProgress?.('extracting');
        const AdmZip = require('adm-zip');
        const zip = new AdmZip(tmpZip);
        tmpExtract = path.join(os.tmpdir(), `nezuko-update-extract-${Date.now()}`);
