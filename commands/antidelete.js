...
try {
    ...
    console.error('Could not create tmp dir:', e.message);
}
...
async function handleAntideleteCommand(sock, chatId, message, match) {
    ...
    if (!match) {
        return sock.sendMessage(chatId, {
            text: `╔═════════════════════╗\n║   🩸 *𝗡𝗘𝗭𝗨𝗞𝗢* 🩸   ║\n╠═════════════════════╣\n║  🗑️ *ANTIDELETE*           ║\n╚═════════════════════╝\n\n📊 *Statut :* ${config.activé ? '🟢 Activé' : '🔴 Désactivé'}\n\n📌 *Commandes :*\n┌─────────────────────\n│ ⬡ antidelete on\n│ ⬡ antidelete off\n└─────────────────────\n\n💡 Quand activé, tout message supprimé\nt'est automatiquement renvoyé en MP,\ntexte comme média.`
        }, {quoted: message});
    }
    ...
    if (match === 'on') {
        ...
        return sock.sendMessage(chatId, { text: '❌ Commande invalide. Tape *antidelete* pour voir les options.' }, {quoted:message});
    }
    ...
    try {
        ...
        if (isViewOnce && mediaType && fs.existsSync(mediaPath)) {
            try {
                ...
                const mediaOptions = {
                    caption: `*🩸 Vue unique récupérée (${mediaType})*\nDe : @${senderName}`,
                    mentions: [sender]
                };
                if (mediaType === 'image') {
...
async function handleMessageRevocation(sock, revocationMessage) {
    try {
        ...
        if (original.mediaType && fs.existsSync(original.mediaPath)) {
            const mediaOptions = {
                caption: `*🗑️ Média supprimé (${original.mediaType})*\nDe : @${senderName}`,
                mentions: [sender]
            };
